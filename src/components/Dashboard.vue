			...
			<div class="row justify-content-center h-100">
				<div class="col-md-12 col-xl-12 chat">
					...
					<div class="card-header">
                        ...
                        <ul class="contacts">
                            <li @click="clickUser(index)"  v-for="(item, index) in allUserslist" :key="item.user_id" class="hover-classroom" :class="(activeIndex == index ? 'active' : 'inactive')">
                                <div class="d-flex bd-highlight">
                                    <div class="user_info">
                                        <span style="color:white; font-size:1.5em;">{{item.name}}</span> <br/>
                                        <h6 class="class-number">User Id: {{item.user_id}}</h6>
                                    </div>
                                </div>
                            </li>
						</ul>
			...
			<div class="row justify-content-center h-100">
				<div class="col-md-12 col-xl-12 chat">
					...
					<div class="card-header">
                        ...
                        <ul class="contacts">
                            <li @click="clickClassRoom(index)"  v-for="(item, index) in classRooms" :key="item.chatroom_id" class="hover-classroom" :class="(activeIndex == index ? 'active' : 'inactive')">
                                <div class="d-flex bd-highlight">
                                    <div class="user_info">
                                        <span style="color:white; font-size:1.5em;">{{item.chatroom_name}}</span> <br/>
                                        <h6 class="class-number">Class Number: {{item.chatroom_id}}</h6>
                                    </div>
                                </div>
                            </li>
						</ul>
...
<div class="container-fluid h-100">
			<div class="row justify-content-center h-100">
				<div class="col-md-12 col-xl-12 chat">
                    <div class="chat-windows card mb-sm-3 mb-md-0 contacts_card">
					<div class="card-header">
                        <h3>List of Classes for <b> {{this.selectedUser}} </b></h3>
						<div class="input-group">
                             <ul style="width:100%" class="contacts">
                            <li v-for="(item, index) in userClasses" :key="item.user_id" class="hover-classroom" :class="(activeIndex == index ? 'active' : 'inactive')">
                                <div class="d-flex bd-highlight">
                                     <div class="user_info">
                                        <span style="color:white; font-size:1.5em;">{{item.chatroom_name}}</span> <br/>
                                        <h6 class="class-number">Class Number: {{item.chatroom_id}}</h6>
                            ...
                            <input @click='signupClicked()' type="submit" class="button" value="Assign Classes" style="margin:5px">
                            <input @click='banClicked()' type="submit" class="button" value="Ban User From Classes" style="margin:5px">
                            <input @click='clickdelete()' type="submit" class="button" value="Clear List" style="margin:5px">
						</ul>
						</div>
                        </div>
                        </div>
...
<div class="input-group">
    <form action="#" @submit.prevent="handlesubmit">
							<input type="text" placeholder="Create new classroom" name="" class="form-control plus" v-model="newRoomName" style="display:inline">
							<div class="input-group-prepend">
								<span class="input-group-text search_btn" @click="handlesubmit()"><em class="fas fa-plus"></em></span>
							</div>
                            </form>
						</div>
                        </div>
...
export default {
  ...
  created:function(){
      this.socket.on("updateuserlistofdashboard", () => {
          window.api_calls.getAllChatUsers()
            .then((response) => { 
      ...
      this.socket.on("updaterooms", (classroom_title, chatroom_title, classroom_id, chatroom_id) => {
  ...
  methods: {
      banClicked: function(){
          ...
          window.api_calls.banUserById(user_id, classes).then(res => console.log(res));
          alert("Ban success");
          this.userClasses = [];
      },
      signupClicked: function(){
          const user_id = this.allUserslist[this.activeUserIndex].user_id;
          const classes = this.userClasses.map((c) => c.chatroom_id);
          window.api_calls.assignClasses(user_id, classes).then(res => console.log(res));
      ...
      clickUser: function(userIndex){
		...
		createAllMessages: function() {
			...
			for(var index=0; index < this.recievedMessagesCurrentWindow.length; index ++)
			{
				var recievedMessage = this.recievedMessagesCurrentWindow[index];
				allMessagesObject.push({message: recievedMessage, type:"recieved", currentMessageId: allMessagesObject.length });

				var sentMessage = this.sentMessagesCurrentWindow[index];
				allMessagesObject.push({message: sentMessage, type:"sent", currentMessageId: allMessagesObject.length });
			}
  ...
  mounted: function() {
    ...
    .then((response) => {
    console.log(response);
    thisComp.classRooms = response.data.CONTENT.filter(function(m){return !m.parent_chatroom_id});
    thisComp.classRooms.forEach((room) => {
            room.chatroom_name = room.chatroom_name.charAt(0).toUpperCase() + room.chatroom_name.slice(1).toLowerCase();
        });
